...
import NoticeBox from '../components/NoticeBox.vue';
  ...
  onMounted(()=>{
    ...
    axios.get('/'+role+'/getAnnouncementList?type=1&page='+currentPage+'&size=8').then(res=>{
  ...
  const handleClick = (tab,event)=>{
    ...
    axios.get('/'+role+'/getAnnouncementList?type='+type+'&page='+currentPage+'&size=8').then(res=>{
        console.log(res.data.data.data);
        annoucementList.value = res.data.data.data
        lastPage.value=res.data.data.lastPage*10
        console.log(lastPage.value);
        // annoucementList.value.splice(0, annoucementList.value.length, ...res.data.data.data);
        console.log(annoucementList.value);
    })
