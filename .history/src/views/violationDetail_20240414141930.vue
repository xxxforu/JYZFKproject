...
const postRequst = axios.create({
  baseURLrl: 'https://ps.gwcampus.cn/api'
})
...
//提交申诉
  function goToAppeal(){
    ...
    formData.append('description',commitDescription.value)
    
    formData.append('illegalBehaviorId',parseInt(sessionStorage.getItem('id')))
    var config={
      method:'post',
      url:'https://ps.gwcampus.cn/api/petrol/appeal',
      headers:{
    ...
    axios(config).then(res=>{
      if(res.data.code==0)
      {
        ...
        formData.delete('commitDescription')
        ElMessage({
          showClose: true,
          message: '申诉发送成功！',
          type: 'success',
...
onUnmounted(() => {
  sessionStorage.setItem('isAppeal',false)
}),
onMounted(()=>{
  if(sessionStorage.getItem('isAppeal')=="true") isAppeal.value = true
  else isAppeal.value = false
  console.log(sessionStorage.getItem('isAppeal'),isAppeal.value);
  appealDate.value = sessionStorage.getItem('appealDate') 
  petrolName.value = sessionStorage.getItem('petrolName')
  description.value = sessionStorage.getItem('description')
  time.value =sessionStorage.getItem('dateTime')
  url.value = sessionStorage.getItem('photo')
  list.value.push(url.value)
  console.log(sessionStorage.getItem('illegalType'));
  if(sessionStorage.getItem('illegalType').length==1){
  switch(sessionStorage.getItem('illegalType')){
    case "1":
...
}else illegalType.value = sessionStorage.getItem('illegalType')
})
...
<template>
  <div id="VDpage">
    <span @click="returnPage">/</span><返回</span>
    ...
    <div class="demo-image__preview">
      <el-image
      style="max-width: 70%; margin:20px 0 0"
      :src="url"
      :zoom-rate="1.2"
      :max-scale="7"
      :min-scale="0.2"
    ...
    <div class="descript">
      <h2 v-if="isAppeal" style="text-align: left;margin:10px 20px">申述内容</h2>
    ...
    <div class="toAppeal" v-if="isAppeal">
