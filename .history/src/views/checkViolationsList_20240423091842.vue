...
const handleExport =()=>{
 ...
 var exporSiteFile = function() {
    let fileId = new Date().getTime(); // 使用当前时间戳作为 fileId
      ...
      axios.get('/central/exportData?startDate='+value1.value[0]+'&endDate='+value1.value[1]+'&pidList='+exportPid,{  responseType: 'blob' }).then(res => {
				if (res.data.size > 0) {
					...
					link.setAttribute("download", value1.value[0]+'To'+value1.value[1]+'By'+exportPid.join('&')+'.xlxs'); // 自定义下载文件名（如exemple.txt）
