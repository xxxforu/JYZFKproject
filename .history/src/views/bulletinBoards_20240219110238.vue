...
import NoticeBox from '../components/NoticeBox.vue';
  ...
  const handleClick = (tab,event)=>{
    ...
    axios.get('/'+role+'/getAnnouncementList?type='+type.value+'&page='+currentPage.value+'&size=8').then(res=>{
        ...
        annoucementList.value =annoucementList.value .concat(annoucementList.value )
        annoucementList.value =annoucementList.value .concat(annoucementList.value )
        
        lastPage.value=res.data.data.lastPage*10*2
        console.log(annoucementList.value.length);
        if(annoucementList.value.length<8) showCutLine.value=false
