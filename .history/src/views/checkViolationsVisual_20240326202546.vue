...
function init1() {
  ...
  eInfo0.value= res.data.data
  console.log(res.data.data);
})
  console.log(eInfo0.value.checking);
  var myChart1 = echarts.init(echart1.value);
  var option1 = {
    title: {
      text: '违规行为的类型分布',
    ...
    series: [
      {
        ...
        radius: '50%',
        data: [
          {value: eInfo0.value.checking, name: '未正确摆放护栏'},
          {value: eInfo0.value.existPerson, name: '卸油区有人'},
          {value: eInfo0.value.fire, name: '发生火灾'},
          {value: eInfo0.value.hat, name: '检修区未戴安全帽'},
          {value: eInfo0.value.phone, name: '打电话'},
        ],
        //.value.map(d => d.value),
        emphasis: {
          itemStyle: {
